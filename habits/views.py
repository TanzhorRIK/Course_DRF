...
class HabitCreateView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        new_habit.task = create_periodic_task(new_habit.frequency, new_habit.pk,
                                              new_habit.time)
        new_habit.save()
...
class HabitUpdateView(UpdateAPIView):
    ...
    def perform_update(self, serializer):
        ...
        update_habit.task = create_periodic_task(update_habit.frequency,
                                                 update_habit.pk,
                                                 update_habit.time)
        update_habit.save()
...
class HabitDeleteView(DestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if len(Habit.objects.filter(link_pleasant=instance)) > 0:
            return Response(
                {'error_message': 'Это связанная привычка не могу удалить'},
